...
},
{
  ...
  "body": " F-test for Variance        At the end of this section you will be able to:     Write hypotheses for the difference in variance    Determine the test statistic and p-value for the F-test.    Write the conclusion of a test in context      Understanding the Basics  The two-sample variance test is used to compare the variances of two populations to determine whether the populations have different amounts of variability. In an introductory statistics course, this test is typically carried out with the -test for two population variances. The hypotheses are written in terms of the population variances, and , often to test whether they are equal.  The test statistic is where and are the sample variances from the two independent samples. Usually, the larger sample variance is placed in the numerator so that the test statistic is at least . This will also make it a right tailed test.  If the null hypothesis is true and the populations are normally distributed, this statistic follows an F-distribution with and degrees of freedom. This test assumes that the two samples are independent, that the observations within each sample are independent, and that both populations are approximately normally distributed. Because the -test is sensitive to departures from normality, the normality assumption is especially important. The resulting p-value is used to decide whether there is enough evidence to conclude that the population variances differ.  Recall that when the p-value  we reject the null hypothesis, otherwise there is not enough evidence to reject the null hypothesis.    A two-variance -test is a statistical test used to compare the variances of two populations and determine whether there is evidence of a difference between them.  The test statistic is   where and are the sample variances from the two independent samples.    introduce F-distribution with statkey    Examples to Compare Variance  A school administrator wants to compare the consistency of travel times to school for students who ride the bus and students who are driven by a parent. To measure consistency, the administrator records the travel time, in minutes, for a random sample of students from each group over several mornings. The summary statistics are shown below.  Bus riders: , minutes  Parent drop-off: , minutes  At the significance level, test whether the population variances in travel time are different for the two groups.   Travel Times   State the hypotheses, compute the statistical test value and p-value, and write the conclusion in context.   Hypotheses  Let population be bus riders and population be parent drop-off students. We want to test whether the population variances are different.  The hypotheses are and .    Statistical Test  The test statistic for a two-variance -test is .  Using the sample standard deviations, .  The degrees of freedom are and .  Using an -distribution with and degrees of freedom, the two-sided -value is approximately .    Conclusion  Since , we fail to reject .  There is not sufficient evidence at the significance level to conclude that the population variances in travel time are different for students who ride the bus and students who are driven by a parent.    A manufacturing engineer wants to compare the consistency of fill amounts for two machines that package bottles of sports drink. A random sample of bottles is taken from each machine, and the amount in each bottle is measured in ounces. The summary statistics are shown below.  Machine A: , ounces  Machine B: , ounces  At the significance level, test whether the population variances in fill amounts are different for the two machines.   Bottle   State the hypotheses, compute the statistical test value and p-value, and write the conclusion in context.    Hypothesis  Let population be bottles filled by Machine A and population be bottles filled by Machine B. We want to test whether the population variances are different.  The hypotheses are and . This is a two tailed test.    Statistical Test  The test statistic is .  Substitute the sample standard deviations: .  The degrees of freedom are and .  Using the -distribution with and degrees of freedom, the two-sided -value is approximately .    Conclusion  Since , we fail to reject .  There is not sufficient evidence at the significance level to conclude that the population variances in fill amounts are different for the two machines.      Using Technology  A hospital administrator wants to determine whether emergency room wait times are more variable on weekends than on weekdays. A random sample of wait times, measured in minutes, is collected for patients arriving on weekends and for patients arriving on weekdays. The summary statistics are shown below.  Weekends: , minutes  Weekdays: , minutes  At the significance level, test whether the population variance in weekend wait times is greater than the population variance in weekday wait times.  Wait Times   State the hypotheses, compute the statistical test value and p-value, and write the conclusion in context.    Hypotheses  Let population be weekend wait times and population be weekday wait times.  We test versus . This is a right tailed test.    Statistical Test  The test statistic for a two-variance -test is .  Substituting the sample standard deviations gives .  The degrees of freedom are and .  Using the -distribution with and degrees of freedom, the one-sided -value is approximately .    Conclusion  Since , we fail to reject .  There is not sufficient evidence at the significance level to conclude that emergency room wait times are more variable on weekends than on weekdays.     "
},
{
  ...
  "number": "8.4",
  "title": "",
  "body": "  At the end of this section you will be able to:     Write hypotheses for the difference in variance    Determine the test statistic and p-value for the F-test.    Write the conclusion of a test in context    "
},
{
...
},
{
  ...
  "number": "8.4.1",
  "title": "",
  "body": "  A two-variance -test is a statistical test used to compare the variances of two populations and determine whether there is evidence of a difference between them.  The test statistic is   where and are the sample variances from the two independent samples.   "
},
{
...
},
{
  ...
  "number": "8.4.3",
  "title": "Bottle.",
  "body": " Bottle   State the hypotheses, compute the statistical test value and p-value, and write the conclusion in context.    Hypothesis  Let population be bottles filled by Machine A and population be bottles filled by Machine B. We want to test whether the population variances are different.  The hypotheses are and . This is a two tailed test.    Statistical Test  The test statistic is .  Substitute the sample standard deviations: .  The degrees of freedom are and .  Using the -distribution with and degrees of freedom, the two-sided -value is approximately .    Conclusion  Since , we fail to reject .  There is not sufficient evidence at the significance level to conclude that the population variances in fill amounts are different for the two machines.   "
},
{
